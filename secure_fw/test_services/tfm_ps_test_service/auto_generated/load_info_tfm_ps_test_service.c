...
#define TFM_SP_PS_TEST_NDEPS                                    (2)
#define TFM_SP_PS_TEST_NSERVS                                   (1)
#if TFM_LVL == 3
#define TFM_SP_PS_TEST_NASSETS                                  (0 + 1)
#else
#define TFM_SP_PS_TEST_NASSETS                                  (0)
#endif
#define TFM_SP_PS_TEST_NIRQS                                    (0)
...
struct partition_tfm_sp_ps_test_load_info_t {
    /* common length load data */
    struct partition_load_info_t    load_info;
    /* per-partition variable length load data */
    uintptr_t                       stack_addr;
    uintptr_t                       heap_addr;
    uint32_t                        deps[TFM_SP_PS_TEST_NDEPS];
    struct service_load_info_t      services[TFM_SP_PS_TEST_NSERVS];
#if TFM_LVL == 3
    struct asset_desc_t             assets[TFM_SP_PS_TEST_NASSETS];
#else
#endif
...
__root
#endif /* __ICCARM__ */
const struct partition_tfm_sp_ps_test_load_info_t tfm_sp_ps_test_load
    ...
    .assets                         = {
        {
            .mem.start              = PART_REGION_ADDR(PT_TFM_SP_PS_TEST_PRIVATE, _DATA_START$$Base),
            .mem.limit              = PART_REGION_ADDR(PT_TFM_SP_PS_TEST_PRIVATE, _DATA_END$$Base),
...
__root
#endif /* __ICCARM__ */
static struct partition_t tfm_sp_ps_test_partition_runtime_item
    __attribute__((used, section(".bss.part_runtime")));
#if defined(__ICCARM__)
#pragma location = ".bss.serv_runtime"
__root
#endif /* __ICCARM__ */
static struct service_t tfm_sp_ps_test_service_runtime_item[TFM_SP_PS_TEST_NSERVS]
