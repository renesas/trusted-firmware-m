...
#define TFM_SP_SLIH_TEST_NDEPS                                  (0)
#define TFM_SP_SLIH_TEST_NSERVS                                 (1)
#if TFM_LVL == 3
#define TFM_SP_SLIH_TEST_NASSETS                                (1 + 1)
#else
#define TFM_SP_SLIH_TEST_NASSETS                                (1)
#endif
#define TFM_SP_SLIH_TEST_NIRQS                                  (1)

/* Memory region declaration */
#if TFM_LVL == 3
REGION_DECLARE(Image$$, PT_TFM_SP_SLIH_TEST_PRIVATE, _DATA_START$$Base);
...
struct partition_tfm_sp_slih_test_load_info_t {
    ...
    uintptr_t                       heap_addr;
    struct service_load_info_t      services[TFM_SP_SLIH_TEST_NSERVS];
#if TFM_LVL == 3
    struct asset_desc_t             assets[TFM_SP_SLIH_TEST_NASSETS];
#else
    struct asset_desc_t             assets[TFM_SP_SLIH_TEST_NASSETS];
#endif
    struct irq_load_info_t          irqs[TFM_SP_SLIH_TEST_NIRQS];
