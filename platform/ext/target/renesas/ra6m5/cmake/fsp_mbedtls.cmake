...
# SCE9 GCM ALT relies on, vendor key plumbing, ML-DSA/ML-KEM - and its rm_psa_crypto *_ALT
...
if(NOT EXISTS "${_fsp_mbedtls}/library/psa_crypto.c")
    message(FATAL_ERROR
        "RA6M5: FSP's Mbed TLS is not in ${_fsp_mbedtls}.\n"
        ...
        "-DRA6M5_FSP_MBEDTLS=OFF to use upstream Mbed TLS.")
endif()
...
if(NOT _fsp_mbedtls_version)
    message(FATAL_ERROR "RA6M5: could not read MBEDTLS_VERSION_STRING from FSP's build_info.h")
endif()
...
if(NOT EXISTS "${_upstream}/CMakeLists.txt")
    message(STATUS "RA6M5: fetching ${_tag} scaffolding for FSP's Mbed TLS")
    execute_process(
        COMMAND ${GIT_EXECUTABLE} clone -q --depth 1 --branch ${_tag}
                ${MBEDCRYPTO_GIT_REMOTE} "${_upstream}"
        RESULT_VARIABLE _rc)
    if(NOT _rc EQUAL 0)
        message(FATAL_ERROR "RA6M5: cloning ${_tag} from ${MBEDCRYPTO_GIT_REMOTE} failed")
    endif()
    execute_process(
        COMMAND ${GIT_EXECUTABLE} -C "${_upstream}" submodule update -q --init --depth 1 framework
        RESULT_VARIABLE _rc)
    if(NOT _rc EQUAL 0)
        message(FATAL_ERROR "RA6M5: fetching the Mbed TLS framework submodule failed")
    endif()
endif()
...
foreach(_p ${_patches})
    ...
    if(NOT _rc EQUAL 0)
        ...
        message(FATAL_ERROR
            "RA6M5: ${_pn} does not apply to FSP's Mbed TLS ${_fsp_mbedtls_version}:\n${_err}\n"
...
message(STATUS "RA6M5: crypto from FSP's Mbed TLS ${_fsp_mbedtls_version} - ${_overlay}")
