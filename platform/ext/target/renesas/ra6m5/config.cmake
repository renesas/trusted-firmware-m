...
#-------------------------------------------------------------------------------
#
# RA6M5 platform configuration.
#
# MCUboot settings. TF-M builds its OWN BL2 from its own vendored MCUboot (bl2/ext/mcuboot)
# and signs with its own keys - the e2 ra6m5_mcuboot project contributes only FSP driver
...
#
# ra6m5_mcuboot's own settings, for the standalone bootloader (2026-08-26):
...
#-------------------------------------------------------------------------------
set(_ra6m5_linker_info "${FSP_S_APP_DIR}/Debug/bsp_linker_info.h")
if(EXISTS "${_ra6m5_linker_info}")
    foreach(_pair "BL2_HEADER_SIZE;___BL_0_P_H_SIZE" "BL2_TRAILER_SIZE;___BL_0_P_T_SIZE")
        ...
        file(STRINGS "${_ra6m5_linker_info}" _hit
    ...
    message(WARNING
        "RA6M5: ${_ra6m5_linker_info} not found while reading the MCUboot header/trailer "
...
# SHOULD be 128 - the RA6M5 code flash minimum write (BSP_FEATURE_FLASH_HP_CF_WRITE_SIZE),
...
# secure image must end by 0x120000-0x180 = 0x11FE80. The NSC is 0x800 at 0x11F800 and the
# veneers are pinned at its start, which leaves the trailer clear.
#
# That 0x800 is not spare generosity - it is what RA6E1 had to be repartitioned to. At an
# 0x400 NSC the veneers sat 0x400 from the slot end and the signed image overran the trailer
# by 17 bytes; imgtool failed the build loudly ("Image size + trailer exceeds requested
# size") rather than producing a silently unbootable image. Carry the 0x800 here.
#
# This value therefore REQUIRES that layout - it is not independent of the solution. If the
# NSC shrinks, or the secure image grows into the space the trailer needs, imgtool fails the
# build again. That is the intended behaviour.
#
# Corresponding Partition Manager values: Code Secure 1150 KB, NSC 2 KB, NS at 0x120000
# (1152 KB). See RA6M5_SOLUTION.md.
#
...
set(MCUBOOT_ALIGN_VAL                   128         CACHE STRING    "RA6M5 code flash write unit; needs the 2 KB NSC layout")
...
# (per-partition MPU regions programmed on each context switch), and the RA6M5 Cortex-M33
...
# the IDAU/SAU. RA6M5_SOLUTION.md carries the RDPM values; they are unaffected by this.
...
set(RA6M5_BL2_HALT_AT_MAIN              OFF         CACHE BOOL      "BL2 spins at main()")
