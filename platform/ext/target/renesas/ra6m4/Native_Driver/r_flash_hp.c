...
#endif
#ifndef BSP_MCU_GROUP_RA6M4
 #define FLASH_HP_DF_START_ADDRESS    (0x40100000)

#else
 #define FLASH_HP_DF_START_ADDRESS    (0x08000000)
#endif
...
static fsp_err_t r_flash_hp_write_read_bc_parameter_checking(flash_hp_instance_ctrl_t * const p_ctrl,
                            ...
                            uint32_t const       num_bytes)
{
    ...
    err = r_flash_hp_write_read_bc_parameter_checking(p_ctrl, flash_address, num_bytes, true);
                                 ...
                                 flash_result_t     * p_blank_check_result)
{
    ...
    err = r_flash_hp_write_read_bc_parameter_checking(p_ctrl, address, num_bytes, false);
    FSP_ERROR_RETURN((err == FSP_SUCCESS), err);
#endif
...
#if (FLASH_HP_CFG_CODE_FLASH_PROGRAMMING_ENABLE == 1) && (BSP_FEATURE_FLASH_HP_SUPPORTS_ID_CODE == 1)
 #if (FLASH_HP_CFG_PARAM_CHECKING_ENABLE)
...
#if (FLASH_HP_CFG_CODE_FLASH_PROGRAMMING_ENABLE == 1) && (BSP_FEATURE_FLASH_HP_SUPPORTS_ACCESS_WINDOW == 1)
 #if (FLASH_HP_CFG_PARAM_CHECKING_ENABLE)
                                                 ...
                                                 bool                       is_temporary)
{
 ...
 #else
        memset(g_configuration_area_data, 0xFF, sizeof(g_configuration_area_data));

        g_configuration_area_data[FLASH_HP_FCU_CONFIG_SET_FAWE_BTFLG_OFFSET] =
            (uint16_t) (((((uint16_t) ~swap_type) & 0x1U) << 15U) | 0x7FFFU);
 #endif
